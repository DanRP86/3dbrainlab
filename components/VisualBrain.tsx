...
const CONCEPTS = [
  { id: 0, label: "PMI", dir: [1, 0.2, 0.5], color: "#8fe9ff" }, // Azul claro corporativo
  { id: 1, label: "IA", dir: [-0.6, 0.8, 0.4], color: "#d5b36a" }, // Oro/Arena
  { id: 2, label: "ENGINEER", dir: [0.4, 0.7, -0.6], color: "#8ea0d8" }, // Azul técnico
  { id: 3, label: "FLEET", dir: [-0.8, -0.4, 0.5], color: "#95d1b3" }, // Verde esmeralda desaturado
  { id: 4, label: "MANAGEMENT", dir: [0.7, -0.6, -0.4], color: "#b29bcb" }, // Lavanda
  { id: 5, label: "REAL ESTATE", dir: [-0.4, -0.8, -0.5], color: "#c2c2c2" }, // Gris mineral/Concreto
  { id: 6, label: "SPORTS", dir: [0.3, 0.1, -0.9], color: "#c98f8f" }, // Rojo coral apagado
  { id: 7, label: "VAN", dir: [0.1, -0.9, 0.8], color: "#d9c6a2" }, // Madera/Arena cálida
];
...
function SemanticFilament({
  ...
  useFrame((state) => {
    ...
    const currentEnergy = material.uniforms.uEnergy.value;
    ...
    if (kind === "probe") {
      ...
      if (onHeadUpdate && material.uniforms.uEnergy.value > 0.03) {
    ...
    <points ref={pointsRef} geometry={geometry} renderOrder={kind === "transfer" ? 4 : 3}>
      <shaderMaterial
        ...
        uniforms={{
          ...
          uThickness: { value: kind === "probe" ? 2.8 : 5.0 },
          uSeed: { value: seed },
        }}
        vertexShader={`
          uniform float uTime;
        ...
        fragmentShader={`
          ...
          void main() {
            ...
            vec3 color = uColor * (0.65 + core * 0.7);
            float alpha = soft * vAlpha * 0.6;
...
function BrainSculpture() {
  ...
  const { fusedGeometry, hotspots } = useMemo(() => {
    ...
    const spots = CONCEPTS.map((concept) => {
      ...
      return {
        ...concept,
        pos: new THREE.Vector3().fromBufferAttribute(positions as THREE.BufferAttribute, closestIdx),
      };
    });
  ...
  return (
    <group scale={2.8}>
      <points geometry={fusedGeometry} renderOrder={2}>
        <shaderMaterial
          ...
          uniforms={{
            ...
            uColorBase: { value: new THREE.Color("#1d1e22") },
            uColorProbe: { value: new THREE.Color("#6e7481") },
            uColorFocus: { value: new THREE.Color("#888") },
          }}
          vertexShader={`
            uniform float uTime;
            ...
            void main() {
              ...
              ) * 0.0035 * mix(0.35, 1.0, edge);

              pos += normalize(position) * sin(uTime * 0.5 + aRandom * 10.0) * 0.0022;

              float probe = 0.0;
              for (int i = 0; i < 3; i++) {
                probe += smoothstep(0.34, 0.0, distance(pos, uProbePos[i])) * uProbeWeight[i];
              }
          ...
          fragmentShader={`
            ...
            void main() {
              ...
              color = mix(color, uColorProbe, vProbe * 0.7);
              color = mix(color, uColorFocus, max(vFocus, vRay));

              float alpha = (0.06 + vProbe * 0.12 + vFocus * 0.22 + vRay * 0.56) * soft;
              color += core * (vRay * 0.45 + vFocus * 0.16);
      ...
      {hotspots.map((spot, i) => {
        const visible = phase === "settle" && i === currentIndex;
        const hinted = (phase === "selecting" || phase === "transfer") && i === selectedIndex;
            ...
            <Html distanceFactor={8} position={[0.14, 0, 0]} center>
              <div
                style={{
                  ...
                  fontStyle: "italic",
                  whiteSpace: "nowrap",
                  opacity: visible ? 0.88 : hinted ? 0.22 : 0,
                  transform: visible
                    ? "translate(14px, -50%)"
                    : hinted
                    ? "translate(8px, -50%)"
